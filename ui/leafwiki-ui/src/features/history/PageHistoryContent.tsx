...
export function PageHistoryContent({
  ...
  const chips = useMemo(() => {
    ...
    const result = [
      `Version slug: ${selectedRevision.slug || '/'}`,
  ...
  const detailLoading =
    activeTab === 'changes' ? compareLoading : previewLoading
    ...
    if (detailLoading && !comparison && !snapshot) {
      return (
        <div className="page-history__loading-state">
          {activeTab === 'changes' ? 'Loading diff...' : 'Loading preview...'}
        </div>
      )
    }
  ...
  const renderRevisionList = () => {
    ...
    if (revisions.length === 0) {
      return (
        <div className="page-history__list-status">
          {latestRevisionId
            ? 'No previous versions yet. Older versions will appear here after more changes.'
            : 'No versions yet. They will appear here after the page changes.'}
        </div>
      )
    }
            ...
            <div className="page-history__header-copy">
              ...
              {selectedRevision ? (
                <div className="page-history__header-subtitle">
                  Version by {displayAuthor(selectedRevision)} ·{' '}
