...
var _ = ginkgo.Describe("leafwiki command helper edges", func() {
	...
	ginkgo.It("formats project daemon identity and role snapshots", func() {
		...
		Expect(projectDaemonDescriptorRoles(projectdaemon.RuntimeStackWikidFrontd, 123, "127.0.0.1:8080", nil)).To(HaveExactElements(
			...
			MatchProjectDaemonRoleHealth(projectdaemon.RoleWorkspaced, projectdaemon.RoleStateReady, gstruct.Fields{
				"PID":     Equal(123),
				"Private": BeTrue(),
			}),
		))
	...
	ginkgo.It("filters native STDIO frames without leaking invalid JSON to the MCP stream", func() {
		...
		Expect(stdout.String()).To(Equal(leafwikiNativeStdioParseErrorFrame + "\n"))
	})
	...
	ginkgo.It("parses startup diagnostics and trusts only local daemon control URLs", func() {
		...
		Expect(structured.RenderedMessage).To(Equal(leafwikiFixtureDaemonStopped))
	...
	ginkgo.It("normalizes daemon runtime config and log paths for owner and workspace requests", func() {
		...
		Expect(err).NotTo(HaveOccurred())
		Expect(daemonCfg).To(gstruct.MatchFields(gstruct.IgnoreExtras, gstruct.Fields{
	...
	ginkgo.It("restarts runtime roles and publishes crash details", func() {
		...
		Expect(crashRuntime.supervisor.State(projectdaemon.RoleFrontd).Error).To(Equal(frontdExitedErr.Error()))
	...
	ginkgo.It("applies daemon auth, registry, descriptor, and private STDIO helper behavior", func() {
		...
		healthy, err = projectDaemonDescriptorHealthy(context.Background(), healthyDesc)
	...
	ginkgo.It("coordinates project daemon locks, descriptors, and foreground waits", func() {
		...
		Expect(struct {
			Held bool
			Free bool
		}{held, free}).To(gstruct.MatchFields(gstruct.IgnoreExtras, gstruct.Fields{
			"Held": BeFalse(),
			"Free": BeFalse(),
		}))
		Expect(dataLock.Release()).To(Succeed())
	...
	ginkgo.It("reports frontd and workspaced role fast failures", func() {
		...
		Expect(runFrontdRole(context.Background(), badWikid)).To(Satisfy(frontd.IsInvalidWikidUpstream))

		badWorkspaced := startup
		badWorkspaced.WorkspacedURL = "http://[::1"
		Expect(runFrontdRole(context.Background(), badWorkspaced)).To(Satisfy(frontd.IsInvalidWorkspacedUpstream))
	...
	ginkgo.It("keeps direct manager, storage, and environment helpers deterministic", func() {
		...
		if hadHome {
			ginkgo.DeferCleanup(os.Setenv, "HOME", oldHome)
		} else {
	...
	ginkgo.It("orchestrates federated attach behavior through daemon boundaries", func() {
		...
		firstContactWorkspace, firstContactIsHome, err := registerFederatedFirstContact(firstContactLayout, projectdaemon.Config{DataDir: leafwikiTempDir(), RootDir: leafwikiTempDir()}, leafwikiRuntimeConfig{
			APIKey:        "lwk_key_missing",
			MCPTransports: mcpTransports{Stdio: true},
			JWTSecret:     "jwt",
			AdminPassword: "admin",
		})
		Expect(firstContactWorkspace).To(Equal(wikid.WorkspaceRecord{}))
		Expect(firstContactIsHome).To(BeFalse())
		Expect(err).To(MatchError(coreauth.ErrInvalidToken))
