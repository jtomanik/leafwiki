...
assert_contains "$api_key_output" "LEAFWIKI_JWT_SECRET=REDACTED" "api-key dry-run"
assert_contains "$api_key_output" "LEAFWIKI_ADMIN_PASSWORD=REDACTED" "api-key dry-run"
...
set +e
bad_host_output="$("$script" --dry-run --host 0.0.0.0 2>&1)"
bad_host_status=$?
set -e
[[ "$bad_host_status" -ne 0 ]] || fail "wildcard host unexpectedly succeeded"
assert_contains "$bad_host_output" "requires a loopback host" "wildcard host error"
...
assert_contains "$(cat "$native_env_file")" "LEAFWIKI_JWT_SECRET=runtime-secret" "native env"
assert_contains "$(cat "$native_env_file")" "LEAFWIKI_ADMIN_PASSWORD=admin" "native env"
