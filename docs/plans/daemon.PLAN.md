...
page:
  ...
  created_at: "2026-06-15T05:44:44.854141263Z"
  updated_at: "2026-06-15T05:44:44.854141263Z"
  creator_id: system
...
- Existing plans: `plans/mcp_transport_unification.PLAN.md`, `plans/stdio.PLAN.md`, `plans/mcp_stdio_sidecar.PLAN.md`, `plans/workspaces.PLAN.md`, `plans/mcp_api_keys.PLAN.md`.

## Implementation Changes

- Add an internal daemon package, for example `internal/projectdaemon`, owning:
  ...
  - detached owner spawn through a hidden internal mode such as `--internal-project-daemon`;
  - config hashing/comparison with secret redaction;
  - session handle registration, heartbeat expiry, idle shutdown, and stale descriptor handling.
...
- Config mismatch rules:
  - Must match for owner-affecting starts: canonical dirs, auth disabled/enabled mode, public HTTP MCP enabled/disabled, host, port, base path, public access, allow-insecure, token timeouts, injected header/custom stylesheet, upload size, revision/link-refactor flags, max revision history, remote-user settings, request-log/log config, idle timeout.
  - STDIO-only attaches inherit the owner's public HTTP MCP, request-log, log target, and log file settings because they do not change the daemon's public surface or logging.
...
```bash
rtk go test ./cmd/leafwiki ./internal/locking ./internal/wiki ./internal/wiki/mcp ./internal/http
rtk go test ./...
