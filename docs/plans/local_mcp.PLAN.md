...
- Add startup validation:
  - allow `enableMCP` with normal auth enabled; authenticated local MCP is protected by OAuth Authorization Code + PKCE
  - keep legacy disabled-auth MCP available when `enableMCP && disableAuth`
