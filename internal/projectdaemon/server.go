...
const (
	...
	errCodeDaemonControlEncodeResponse        sharederrors.ErrorCode = "daemon_control_encode_response"
)
...
func (s *ControlServer) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	if req.Header.Get(ControlTokenHeader) != s.token {
		writeControlError(w, http.StatusUnauthorized, errCodeDaemonControlUnauthorized)
		return
	}
	...
	case req.Method == http.MethodPost && path == "/sessions":
		...
		if err != nil {
			writeControlError(w, http.StatusInternalServerError, errCodeDaemonSessionRegisterFailed)
			return
		}
	...
	case req.Method == http.MethodPost && strings.HasPrefix(path, "/sessions/") && strings.HasSuffix(path, "/heartbeat"):
		...
		if !s.sessions.Heartbeat(SessionIDFromString(id)) {
			writeControlError(w, http.StatusNotFound, errCodeDaemonSessionNotFound)
			return
		}
		writeJSON(w, map[string]any{"ok": true})
	case req.Method == http.MethodDelete && strings.HasPrefix(path, "/sessions/"):
		id := strings.TrimPrefix(path, "/sessions/")
		s.sessions.Release(SessionIDFromString(id))
		writeJSON(w, map[string]any{"ok": true})
...
func (s *ControlServer) recordAgentPresence(w http.ResponseWriter, req *http.Request) {
	...
	if err := json.NewDecoder(req.Body).Decode(&event); err != nil {
		writeControlError(w, http.StatusBadRequest, errCodeDaemonAgentPresenceInvalidRequest)
		return
	}
...
func (s *ControlServer) verifyStdioAuth(w http.ResponseWriter, req *http.Request) {
	...
	if err := json.NewDecoder(req.Body).Decode(&body); err != nil {
		writeControlError(w, http.StatusBadRequest, errCodeStdioAuthInvalidRequest)
		return
	}
	if s.authDisabled {
		if strings.TrimSpace(body.APIKey) != "" {
			writeControlError(w, http.StatusConflict, errCodeStdioAuthAPIKeyRejected)
			return
		}
		writeJSON(w, map[string]any{"ok": true})
		return
	...
	if strings.TrimSpace(body.APIKey) == "" {
		writeControlError(w, http.StatusUnauthorized, errCodeStdioAuthAPIKeyRequired)
		return
	}
	if s.verifyAPIKey == nil {
		writeControlError(w, http.StatusInternalServerError, errCodeStdioAuthAPIKeyVerifierUnavailable)
		return
	}
	if err := s.verifyAPIKey(body.APIKey); err != nil {
		if errors.Is(err, ErrInvalidAPIKey) {
			writeControlError(w, http.StatusUnauthorized, errCodeStdioAuthAPIKeyInvalid)
			return
		}
		writeControlError(w, http.StatusServiceUnavailable, errCodeStdioAuthAPIKeyVerifierFailed)
		return
	}
	writeJSON(w, map[string]any{"ok": true})
}

func writeControlError(w http.ResponseWriter, status int, code sharederrors.ErrorCode) {
...
func writeJSON(w http.ResponseWriter, value any) {
	...
	if err := json.NewEncoder(w).Encode(value); err != nil {
		http.Error(w, sharederrors.NewLocalizedErrorDetailFromCode(errCodeDaemonControlEncodeResponse).Message, http.StatusInternalServerError)
	}
}
