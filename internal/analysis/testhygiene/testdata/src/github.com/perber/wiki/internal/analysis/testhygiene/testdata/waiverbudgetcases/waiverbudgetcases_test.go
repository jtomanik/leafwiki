...
// semh:allow ginkgo.top-level-it -- fourth package-level invariant // want "semh:waiver.budget-exceeded: waiver budget exceeded for ginkgo.top-level-it: used 4, budget 3"
var _ = ginkgo.It("documents fourth package invariant", ginkgo.Label("unit"), func() {})
...
func equalMatcherBudgetCases() {
	...
	Expect([]string{}).To(Equal([]string{}))
	// semh:allow gomega.equal-zero -- first zero matcher exception // want "semh:waiver.budget-exceeded: total waiver budget exceeded: used 11, budget 10"
	Expect(0).To(Equal(0))
