...
var _ = ginkgo.Describe("markdown link resolution", ginkgo.Label("unit"), func() {
	ginkgo.When("links point at pages and sections", func() {
		ginkgo.It("resolves relative page links from the source file directory", func() {
			...
			index := NewIndex([]Entry{
				{Kind: EntryKindPage, Path: newFixtureMarkdownPath("docs/b/target.md")},
			})

			result := index.Resolve(newFixtureMarkdownPath("docs/a/current.md"), "../b/target")

			Expect(result).To(matchResolvedLink(TargetKindPage, "../b/target.md", newFixtureRoutePath("docs/b/target")))
		})
		...
		ginkgo.It("canonicalizes a trailing slash section destination without changing the section route", func() {
			...
			index := NewIndex([]Entry{
				{Kind: EntryKindSection, Path: newFixtureMarkdownPath("docs/sync")},
			})

			result := index.Resolve(newFixtureMarkdownPath("docs/a.md"), "/docs/sync/")

			Expect(result).To(matchResolvedLink(TargetKindSection, "/docs/sync", newFixtureRoutePath("docs/sync")))
		})
		...
		ginkgo.It("resolves a relative root section destination to a slash href", func() {
			...
			index := NewIndex([]Entry{
				{Kind: EntryKindSection, Path: newFixtureMarkdownPath(""), ContentPath: newFixtureMarkdownPath("index.md")},
			})

			result := index.Resolve(newFixtureMarkdownPath("docs/nested/page.md"), "../..")
		...
		ginkgo.It("keeps explicit README page links as pages when a section fallback also exists", func() {
			index := NewIndex([]Entry{
				{Kind: EntryKindSection, Path: newFixtureMarkdownPath("docs/sync"), ContentPath: newFixtureMarkdownPath("docs/sync/index.md")},
				{Kind: EntryKindSection, Path: newFixtureMarkdownPath("guides"), ContentPath: newFixtureMarkdownPath("guides/README.md")},
				{Kind: EntryKindPage, Path: newFixtureMarkdownPath("docs/sync/README.md")},
			})

			readmePage := index.Resolve(newFixtureMarkdownPath("docs/a.md"), "/docs/sync/README.md")

			Expect(readmePage).To(matchResolvedLink(TargetKindPage, "/docs/sync/README.md", newFixtureRoutePath("docs/sync/README")))
		})

		ginkgo.It("prefers the canonical section when page and section routes share an extensionless link", func() {
			index := NewIndex([]Entry{
				{Kind: EntryKindPage, Path: newFixtureMarkdownPath("docs/sync.md")},
				{Kind: EntryKindSection, Path: newFixtureMarkdownPath("docs/sync"), ContentPath: newFixtureMarkdownPath("docs/sync/index.md")},
			})
	...
	ginkgo.When("a markdown link root prefix is configured", func() {
		ginkgo.It("resolves prefixed absolute page links inside the wiki root", func() {
			...
			index := NewIndexWithOptions([]Entry{
				{Kind: EntryKindPage, Path: newFixtureMarkdownPath("sync/glossary.md")},
			}, Options{MarkdownLinkRootPrefix: "/docs"})

			result := index.Resolve(newFixtureMarkdownPath("index.md"), "/docs/sync/glossary.md")

			Expect(result).To(matchResolvedLink(TargetKindPage, "/docs/sync/glossary.md", newFixtureRoutePath("sync/glossary")))
		})
		...
		ginkgo.It("canonicalizes unprefixed absolute migration links into the configured root prefix", func() {
			...
			result := index.ResolveForMigration(newFixtureMarkdownPath("index.md"), "/sync/glossary?view=1#term")

			Expect(result).To(matchResolvedLink(TargetKindPage, "/docs/sync/glossary.md?view=1#term", newFixtureRoutePath("sync/glossary")))
		})
		...
		ginkgo.DescribeTable("resolves the configured prefix root to the wiki root section",
			func(href string) {
				...
				index := NewIndexWithOptions([]Entry{
					{Kind: EntryKindSection, Path: newFixtureMarkdownPath("")},
				}, Options{MarkdownLinkRootPrefix: "/docs"})

				result := index.Resolve(newFixtureMarkdownPath("index.md"), href)
		...
		ginkgo.It("distinguishes extensionless sections from explicit markdown pages", func() {
			...
			index := NewIndexWithOptions([]Entry{
				{Kind: EntryKindSection, Path: newFixtureMarkdownPath("sync"), ContentPath: newFixtureMarkdownPath("sync/index.md")},
				{Kind: EntryKindPage, Path: newFixtureMarkdownPath("sync.md")},
			}, Options{MarkdownLinkRootPrefix: "/docs"})

			section := index.Resolve(newFixtureMarkdownPath("index.md"), "/docs/sync")
			page := index.Resolve(newFixtureMarkdownPath("index.md"), "/docs/sync.md")

			Expect(section).To(matchResolvedLink(TargetKindSection, "/docs/sync", newFixtureRoutePath("sync")))
			Expect(page).To(matchResolvedLink(TargetKindPage, "/docs/sync.md", newFixtureRoutePath("sync")))
		})
		...
		ginkgo.It("leaves relative links canonical relative to the source file", func() {
			...
			index := NewIndexWithOptions([]Entry{
				{Kind: EntryKindPage, Path: newFixtureMarkdownPath("glossary.md")},
			}, Options{MarkdownLinkRootPrefix: "/docs"})

			relative := index.Resolve(newFixtureMarkdownPath("sync/page.md"), "../glossary.md")

			Expect(relative).To(matchResolvedLink(TargetKindPage, "../glossary.md", newFixtureRoutePath("glossary")))
		})
		...
		ginkgo.DescribeTable("leaves external and hash links unchanged",
			func(href string) {
				...
				result := index.Resolve(newFixtureMarkdownPath("sync/page.md"), href)

				Expect(result).To(matchResolvedLink(TargetKindExternal, href, newFixtureRoutePath("")))
			},
		...
		ginkgo.It("resolves prefixed asset destinations as asset links", func() {
			...
			result := index.Resolve(newFixtureMarkdownPath("index.md"), "/docs/assets/logo.png")

			Expect(result).To(matchResolvedLink(TargetKindAsset, "/docs/assets/logo.png", newFixtureRoutePath("")))
		})
	})
	...
	ginkgo.When("classifying unresolved and non-page destinations", func() {
		ginkgo.DescribeTable("reports the target kind and issue code for canonical resolution",
			func(href string, kind TargetKind, code IssueCode) {
				index := NewIndex([]Entry{
					{Kind: EntryKindPage, Path: newFixtureMarkdownPath("docs/b.md")},
					{Kind: EntryKindSection, Path: newFixtureMarkdownPath("docs/sync"), ContentPath: newFixtureMarkdownPath("docs/sync/index.md")},
					{Kind: EntryKindAsset, Path: newFixtureMarkdownPath("assets/page/logo.png")},
				})

				result := index.Resolve(newFixtureMarkdownPath("docs/a/current.md"), href)

				Expect(result).To(matchResolution(kind, code))
			},
			ginkgo.Entry("canonical page", "/docs/b.md", TargetKindPage, newFixtureIssueCode("")),
			ginkgo.Entry("canonical section", "/docs/sync", TargetKindSection, newFixtureIssueCode("")),
			ginkgo.Entry("asset namespace", "/assets/page/logo.png", TargetKindAsset, newFixtureIssueCode("")),
			ginkgo.Entry("asset extension", "/docs/manual.pdf", TargetKindAsset, newFixtureIssueCode("")),
			ginkgo.Entry("external URL", "https://example.com", TargetKindExternal, newFixtureIssueCode("")),
			ginkgo.Entry("mailto URL", "mailto:a@example.com", TargetKindExternal, newFixtureIssueCode("")),
			ginkgo.Entry("same-page hash", "#heading", TargetKindExternal, newFixtureIssueCode("")),
		...
		ginkgo.DescribeTable("treats protocol-relative and schemed URLs as external destinations",
			func(href string) {
				...
				index := NewIndex([]Entry{
					{Kind: EntryKindPage, Path: newFixtureMarkdownPath("cdn.example.com/lib.md")},
				})

				result := index.Resolve(newFixtureMarkdownPath("docs/a.md"), href)
		...
		ginkgo.It("keeps percent-encoded path style while matching decoded filesystem paths", func() {
			...
			index := NewIndex([]Entry{
				{Kind: EntryKindPage, Path: newFixtureMarkdownPath("docs/space name.md")},
			})

			result := index.Resolve(newFixtureMarkdownPath("docs/a.md"), "/docs/space%20name?x=1#part")

			Expect(result).To(matchResolvedLink(TargetKindPage, "/docs/space%20name.md?x=1#part", newFixtureRoutePath("docs/space name")))
		})
	})
	...
	ginkgo.When("migration mode handles legacy page and section ambiguity", func() {
		ginkgo.It("leaves ambiguous extensionless page and section twins unresolved", func() {
			...
			result := index.ResolveForMigration(newFixtureMarkdownPath("docs/a.md"), "/docs/sync")
	...
	ginkgo.When("explicit section content files are linked", func() {
		ginkgo.DescribeTable("canonicalizes default section files to their section routes",
			func(href string, canonicalHref string, routePath tree.RoutePath) {
			...
			ginkgo.Entry("index markdown file", "/docs/sync/index.md", "/docs/sync", newFixtureRoutePath("docs/sync")),
			ginkgo.Entry("percent-encoded index markdown file", "/docs/%73ync/index.md", "/docs/%73ync", newFixtureRoutePath("docs/sync")),
			ginkgo.Entry("README markdown file", "/guides/README.md", "/guides", newFixtureRoutePath("guides")),
			ginkgo.Entry("percent-encoded README markdown file", "/guid%65s/README.md", "/guid%65s", newFixtureRoutePath("guides")),
		)
	})
	...
	ginkgo.When("building an index from workspace files", func() {
		ginkgo.It("uses workspace route normalization for canonical route paths", func() {
			...
			result := index.Resolve(newFixtureMarkdownPath("source.md"), "/plans/agent-hooks-plan.md")
			rawAbsolute := index.Resolve(newFixtureMarkdownPath("source.md"), "/plans/agent_hooks.PLAN.md")
			rawRelative := index.Resolve(newFixtureMarkdownPath("plans/source.md"), "./agent_hooks.PLAN.md")
			migration := index.ResolveForMigration(newFixtureMarkdownPath("plans/source.md"), "./agent_hooks.PLAN.md")

			Expect(result).To(matchResolvedLink(TargetKindPage, "/plans/agent-hooks-plan.md", newFixtureRoutePath("plans/agent-hooks-plan")))
			...
			Expect(migration).To(matchResolvedLink(TargetKindPage, "agent-hooks-plan.md", newFixtureRoutePath("plans/agent-hooks-plan")))
		})
		...
		ginkgo.It("applies markdown link root prefixes to discovered page canonical hrefs", func() {
			...
			result := index.Resolve(newFixtureMarkdownPath("source.md"), "/wiki/docs/page.md")

			Expect(result).To(matchResolvedLink(TargetKindPage, "/wiki/docs/page.md", newFixtureRoutePath("docs/page")))
		})
	})
})
...
var _ = ginkgo.Describe("markdown link rewriting", ginkgo.Label("unit"), func() {
	ginkgo.When("rewriting inline and reference markdown destinations", func() {
		ginkgo.It("preserves angle destinations, titles, query strings, and fragments", func() {
			...
			result := index.RewriteMarkdown(newFixtureMarkdownPath("docs/a.md"), `[B](</docs/b?mode=raw#part-two> "open B")`)
		...
		ginkgo.It("rewrites reference definitions without touching code spans or fenced code blocks", func() {
			...
			result := index.RewriteMarkdown(newFixtureMarkdownPath("docs/a.md"), content)
...
[Real](/docs/b.md)`
	...
	ginkgo.When("content has no canonical page rewrite", func() {
		ginkgo.It("leaves empty content unchanged without issues", func() {
			...
			result := index.RewriteMarkdown(newFixtureMarkdownPath("index.md"), "")
		...
		ginkgo.It("reports invalid and unresolved destinations without changing content", func() {
			...
			result := index.RewriteMarkdown(newFixtureMarkdownPath("index.md"), content)
