...
var _ = ginkgo.Describe("revision store persistence behavior", func() {
	...
	ginkgo.It("PruneRevisions keeps the newest revisions and removes pruned IDs from the index", ginkgo.Label("unit"), func() {
	...
	ginkgo.It("PruneRevisions keep count boundaries are no-ops for zero and already-small histories", ginkgo.Label("unit"), func() {
		store := NewFSStore(revisionTempDir())
	...
	ginkgo.It("CopyAssetBlobToPath restores a stored asset blob with matching hash and size", ginkgo.Label("unit"), func() {
		tmp := revisionTempDir()
