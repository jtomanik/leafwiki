...
func TestContentTransformer_TransformContent_TableDriven(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := transformer.TransformContent(tree.NewUserIDUnchecked("editor"), tree.NewWorkspaceSourcePathUnchecked(tt.sourcePath), page, tt.content, wiki)
			if err != nil {
...
func TestContentTransformer_EmitsMdForPagesAndExtensionlessForSections(t *testing.T) {
	tmp := t.TempDir()
...
func TestContentTransformer_SourcePathSuffixMatchingIsCaseSensitive(t *testing.T) {
	tmp := t.TempDir()
	writeTmp(t, tmp, "current.md", "# Current")
...
func TestContentTransformer_BasenameWikiLinkMatchingIsCaseSensitive(t *testing.T) {
	tmp := t.TempDir()
	writeTmp(t, tmp, "current.md", "# Current")
	writeTmp(t, tmp, "Guide.md", "# Guide")
...
func TestContentTransformer_DoesNotResolveArbitraryMarkdownPageLinkToSection(t *testing.T) {
	tmp := t.TempDir()
	writeTmp(t, tmp, "current.md", "# Current")
	writeTmp(t, tmp, "Guide/index.md", "# Guide")
...
func TestContentTransformer_FormatsSameRoutePageAndSectionTargetsByMatchedKind(t *testing.T) {
	tmp := t.TempDir()
	writeTmp(t, tmp, "current.md", "# Current")
...
func TestContentTransformer_FormatsSameRouteSuffixFallbackByRequestedKind(t *testing.T) {
	tmp := t.TempDir()
	writeTmp(t, tmp, "current.md", "# Current")
...
func TestContentTransformer_ResolvesReadmeFallbackFolderLinks(t *testing.T) {
	tmp := t.TempDir()
	writeTmp(t, tmp, "current.md", "# Current")
...
// - Reference-style link definitions are rewritten
func TestContentTransformer_RewritesReferenceDefinitions(t *testing.T) {
	tmp := t.TempDir()
	writeTmp(t, tmp, "current.md", "# Current")
	writeTmp(t, tmp, "Note.md", "# Note")
