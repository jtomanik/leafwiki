...
var _ = Describe("MCP context checkpoint retention", Label("unit"), func() {
	It("defaults limits and fills generated fields", func() {
		store := newContextCheckpointStore(0)
		before := time.Now().UTC()
		session := contextSessionScopeForTest(newFixtureContextSessionID("session"))
	...
	It("honors disabled pruning and session limits", func() {
		...
		store.ttl = 0
		store.maxSessions = 0
		emptySession := contextSessionScopeForTest(newFixtureContextSessionID("empty"))
		...
		store.sessions[emptySession] = nil
		store.sessions[oldSession] = []contextCheckpoint{{Token: "old", CreatedAt: base.Add(-time.Hour)}}

		store.pruneExpiredLocked(base)
		store.evictOverflowLocked(oldSession)
		store.evictForNewSessionLocked(newSession)

		Expect(store.sessions).To(HaveKey(emptySession))
		Expect(store.sessions).To(HaveKey(oldSession))

		onlyStore := newContextCheckpointStore(2)
		onlySession := contextSessionScopeForTest(newFixtureContextSessionID("only"))
		onlyStore.sessions[onlySession] = []contextCheckpoint{{Token: "only", CreatedAt: base}}

		Expect(checkpointEvictionDecisionFor(onlyStore.evictOldestSessionLocked(onlySession))).To(Equal(checkpointSessionRetained))
	...
	It("prunes expired checkpoints without deleting empty sessions", func() {
		...
		emptySession := contextSessionScopeForTest(newFixtureContextSessionID("empty"))
		expiredSession := contextSessionScopeForTest(newFixtureContextSessionID("expired"))
		mixedSession := contextSessionScopeForTest(newFixtureContextSessionID("mixed"))
		store.sessions[emptySession] = nil
		store.sessions[expiredSession] = []contextCheckpoint{
			{Token: "expired", CreatedAt: base.Add(-2 * time.Minute)},
		}
		store.sessions[mixedSession] = []contextCheckpoint{
		...
		Expect(store.sessions).To(HaveKey(emptySession))
		Expect(store.sessions).NotTo(HaveKey(expiredSession))
		Expect(store.sessions).To(HaveKeyWithValue(mixedSession, []contextCheckpoint{
			{Token: "fresh", CreatedAt: base.Add(-30 * time.Second)},
		}))
	})
})

var _ = Describe("MCP request actor authentication", Label("unit"), func() {
...
var _ = Describe("MCP HTTP route authentication", Label("integration"), func() {
...
var _ = Describe("MCP context state and recent change discovery", func() {
	It("reports default sync, session, actor, and formatting values", Label("unit"), func() {
		...
		Expect(contextSessionKey(nil, toolActor{ID: newFixtureUserID("actor-1")})).To(Equal(contextSessionScope{
			ActorID:   newFixtureUserID("actor-1"),
			SessionID: newFixtureContextSessionID("sessionless"),
		}))
	...
	It("reports commit deltas across pagination outcomes", Label("integration"), func() {
		routes := newContextToolTestRoutes()
		status := workspacesync.SyncStatus{LastCommitHash: newFixtureCommitHash("latest")}

		Expect(changesSinceCommitOutcomeFor(routes, status, newFixtureCommitHash("missing"))).To(matchChangesSinceCommit(
		...
		routes.listWorkspaceSnapshots = func(_ context.Context, cursor workspacesync.CommitHash, limit workspacesync.SnapshotLimit) (workspacesync.SnapshotList, error) {
			...
			case "":
				return workspacesync.SnapshotList{
					Snapshots:  []workspacesync.Snapshot{{ID: newFixtureCommitHash("newer"), ChangedMarkdownPaths: []string{"home.md"}}},
					NextCursor: newFixtureCommitHash("cursor-2"),
				}, nil
			case newFixtureCommitHash("cursor-2"):
				return workspacesync.SnapshotList{
					Snapshots: []workspacesync.Snapshot{{ID: newFixtureCommitHash("target")}},
				}, nil
			default:
		...
		completeChanges := changesSinceCommitOutcomeFor(routes, status, newFixtureCommitHash("target"))
	...
	It("maps root and section route changes to page IDs", Label("integration"), func() {
		routes := newContextToolTestRoutes()
		...
		Expect(routes.pageIDForRecentChangeRoute(newFixtureRoutePath(""), tree.NodeKindSection)).To(Equal(tree.RootPageID))
		Expect(routes.pageIDForRecentChangeRoute(newFixtureRoutePath("guide"), newFixtureNodeKind(""))).To(Equal(*sectionID))
	})

	It("returns no page IDs for unavailable trees and invalid paths", Label("integration"), func() {
		routes := newContextToolTestRoutes()
...
var _ = Describe("MCP validation source and asset resolution", func() {
	It("caches asset predicates and treats nil factories as absent assets", Label("unit"), func() {
		...
		Expect(cachedValidationAssetDestinationFor(exists, newFixturePageID(""), "hit.png")).To(matchValidationAssetPresence(validationAssetAbsent, Equal("hit.png")))
	...
	It("normalizes content validation paths and source kinds", Label("integration"), func() {
		...
		sectionID, err := routes.treeService.CreateNode(newFixtureUserID("system"), nil, "Guide", newFixtureSlug("guide"), testNodeKindPtr(tree.NodeKindSection))
		Expect(err).NotTo(HaveOccurred())
		pageID, err := routes.treeService.CreateNode(newFixtureUserID("system"), nil, "Guide Page", newFixtureSlug("guide-page"), testNodeKindPtr(tree.NodeKindPage))
		Expect(err).NotTo(HaveOccurred())

		Expect((&Routes{}).validationSourceKind(newFixturePageID("missing"))).To(Equal(tree.NodeKindPage))
		Expect(routes.validationSourceKind(newFixturePageID(""))).To(Equal(tree.NodeKindPage))
		...
		Expect(routes.validationSourceKindForRoute(newFixtureRoutePath(""))).To(Equal(tree.NodeKindPage))
		...
		Expect(routes.validationSourceMarkdownFile(newFixtureRoutePath("guide"))).To(Equal(newFixtureMarkdownPath("guide/index.md")))
		Expect((&Routes{}).validationSourceMarkdownFile(newFixtureRoutePath("guide"))).To(Equal(newFixtureMarkdownPath("guide.md")))
		...
		Expect(validationPageIDResolutionFor(routes, newFixtureRoutePath(""))).To(matchValidationPageID(validationUnresolved, BeEmpty()))
		...
		routePath, kind, err := routes.normalizeValidationContentPathInput("guide.md", newFixtureNodeKind(""))
		Expect(err).NotTo(HaveOccurred())
	...
	It("cleans validation asset destinations and resolves root markdown targets", Label("integration"), func() {
		...
		Expect(markdownTargetNodeKind(newFixtureTargetKind("unexpected"))).To(Equal(tree.NodeKindPage))

		Expect(validationMarkdownLinkResolutionFor(routes, newFixtureRoutePath(""), tree.NodeKindSection, "/")).To(matchValidationMarkdownLink(
			validationResolved,
			BeEmpty(),
...
var _ = Describe("MCP subtree rendering limits", func() {
	It("returns empty subtree results for nil nodes and enforces depth bounds", Label("unit"), func() {
	...
	It("builds content previews and reports truncation at the requested depth", Label("integration"), func() {
		routes := newContextToolTestRoutes()
		parentID, err := routes.treeService.CreateNode(newFixtureUserID("system"), nil, "Parent", newFixtureSlug("parent"), testNodeKindPtr(tree.NodeKindSection))
		Expect(err).NotTo(HaveOccurred())
		childID, err := routes.treeService.CreateNode(newFixtureUserID("system"), parentID, "Child", newFixtureSlug("child"), testNodeKindPtr(tree.NodeKindPage))
		Expect(err).NotTo(HaveOccurred())
		page, err := routes.treeService.GetPage(*childID)
		Expect(err).NotTo(HaveOccurred())
		content := "one\n\n two\tthree"
		Expect(routes.treeService.UpdateNodeUncheckedVersion(newFixtureUserID("system"), page.ID, page.Title, page.Slug, &content, false)).To(Succeed())
		...
		Expect(routes.treeService.UpdateNodeUncheckedVersion(newFixtureUserID("system"), page.ID, page.Title, page.Slug, &longContent, false)).To(Succeed())
...
var _ = Describe("MCP JSON and multipart input decoding", Label("unit"), func() {
