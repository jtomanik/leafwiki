...
func TestAssetUseCaseInputsUseSemanticIDs(t *testing.T) {
	...
	upload := UploadAssetInput{UserID: tree.UserID("user-1"), PageID: tree.NewPageIDUnchecked("page-1"), Filename: tree.AssetName("diagram.png")}
	var _ tree.UserID = upload.UserID
	var _ tree.PageID = upload.PageID
	var _ tree.AssetName = upload.Filename

	list := ListAssetsInput{PageID: tree.NewPageIDUnchecked("page-1")}
	var _ tree.PageID = list.PageID

	get := GetAssetInput{PageID: tree.NewPageIDUnchecked("page-1"), Filename: tree.AssetName("diagram.png")}
	...
	rename := RenameAssetInput{UserID: tree.UserID("user-1"), PageID: tree.NewPageIDUnchecked("page-1"), OldFilename: tree.AssetName("old.png"), NewFilename: tree.AssetName("new.png")}
	...
	deleteInput := DeleteAssetInput{UserID: tree.UserID("user-1"), PageID: tree.NewPageIDUnchecked("page-1"), Filename: tree.AssetName("diagram.png")}
