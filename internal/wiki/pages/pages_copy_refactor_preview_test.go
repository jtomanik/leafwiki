...
var _ = ginkgo.Describe("page use case behavior", ginkgo.Label("integration"), func() {
	ginkgo.It("copies a page", func() {
		...
		out, err := copyUC.Execute(context.Background(), pages.CopyPageInput{
			UserID: newFixtureUserID("user1"), SourcePageID: pageID(original.Page.ID), Title: "Copy of Original", Slug: newFixtureSlug("copy-of-original"),
		})
	...
	ginkgo.It("copies a page under a parent", func() {
		...
		out, err := copyUC.Execute(context.Background(), pages.CopyPageInput{
			UserID: newFixtureUserID("user1"), SourcePageID: pageID(original.Page.ID), TargetParentID: pageIDPtr(parent.Page.ID), Title: "Copy of Original", Slug: newFixtureSlug("copy-of-original"),
		})
		Expect(err).To(Succeed())
	...
	ginkgo.It("rejects copying a missing source page", func() {
		...
		_, err := copyUC.Execute(context.Background(), pages.CopyPageInput{
			UserID: newFixtureUserID("user1"), SourcePageID: newFixturePageID("non-existent-id"), Title: "Copy", Slug: newFixtureSlug("copy"),
		})
	...
	ginkgo.It("copies page assets", func() {
		...
		_, err = deps.assets.SaveAssetForPage(original.Page.PageNode, file, newFixtureAssetName("image.png"), 1024)
		Expect(err).To(Succeed())
	...
	ginkgo.It("indexes outgoing links for copied pages", func() {
		...
		target, err := createUC.Execute(context.Background(), pages.CreatePageInput{
			UserID: newFixtureUserID("user1"), Title: "Target", Slug: newFixtureSlug("target"), Kind: pageKind(),
		})
		Expect(err).To(Succeed())
		...
		if _, err := updateUC.Execute(context.Background(), pages.UpdatePageInput{
			UserID: newFixtureUserID("user1"), ID: pageID(original.Page.ID), Version: pageVersion(original.Page.Version()), Title: original.Page.Title, Slug: slug(original.Page.Slug), Content: &content, Kind: pageKind(),
		}); err != nil {
			Expect(err).To(Succeed())
		}

		out, err := copyUC.Execute(context.Background(), pages.CopyPageInput{
			UserID: newFixtureUserID("user1"), SourcePageID: pageID(original.Page.ID), Title: "Copy", Slug: newFixtureSlug("copy"),
		})
		Expect(err).To(Succeed())
		...
		Expect(outgoing).To(SatisfyAll(
			...
			HaveField("Outgoings", HaveExactElements(HaveHealthyOutgoing(newFixtureRoutePath("/target"), target.Page.ID))),
		))
	})
	...
	ginkgo.It("omits live before snapshots from update events", func() {
		...
		created, err := createUC.Execute(context.Background(), pages.CreatePageInput{
			UserID: newFixtureUserID("user1"), Title: "Old", Slug: newFixtureSlug("old"), Kind: pageKind(),
		})
		Expect(err).To(Succeed())

		content := "updated"
		if _, err := updateUC.Execute(context.Background(), pages.UpdatePageInput{
			UserID: newFixtureUserID("user1"), ID: pageID(created.Page.ID), Version: pageVersion(created.Page.Version()), Title: "New", Slug: newFixtureSlug("new"), Content: &content, Kind: pageKind(),
		}); err != nil {
			Expect(err).To(Succeed())
		}
		...
		Expect(effect.events).To(SatisfyAll(
			...
			ContainElement(SatisfyAll(
				...
				HaveField("OldPath", Equal(newFixtureRoutePath("old"))),
			)),
		))
	})
	...
	ginkgo.It("omits live before snapshots from move events", func() {
		...
		parentA, err := createUC.Execute(context.Background(), pages.CreatePageInput{
			UserID: newFixtureUserID("user1"), Title: "A", Slug: newFixtureSlug("a"), Kind: sectionKind(),
		})
		Expect(err).To(Succeed())
		parentB, err := createUC.Execute(context.Background(), pages.CreatePageInput{
			UserID: newFixtureUserID("user1"), Title: "B", Slug: newFixtureSlug("b"), Kind: sectionKind(),
		})
		Expect(err).To(Succeed())
		child, err := createUC.Execute(context.Background(), pages.CreatePageInput{
			UserID: newFixtureUserID("user1"), ParentID: pageIDPtr(parentA.Page.ID), Title: "Child", Slug: newFixtureSlug("child"), Kind: pageKind(),
		})
		Expect(err).To(Succeed())

		if err := moveUC.Execute(context.Background(), pages.MovePageInput{
			UserID: newFixtureUserID("user1"), ID: pageID(child.Page.ID), Version: pageVersion(child.Page.Version()), ParentID: pageID(parentB.Page.ID),
		}); err != nil {
			Expect(err).To(Succeed())
		}
		...
		Expect(effect.events).To(SatisfyAll(
			...
			ContainElement(SatisfyAll(
				...
				HaveField("Before", BeNil()),
				HaveField("OldPath", Equal(newFixtureRoutePath("a/child"))),
			)),
		))
	})
	...
	ginkgo.It("previews affected pages for page renames", func() {
		...
		Expect(err).To(Succeed())
		Expect(preview.OldPath).To(Equal(newFixtureRoutePath("target")))
	...
	ginkgo.It("ignores section-twin descendants when previewing a page rename", func() {
		...
		docs, err := createUC.Execute(context.Background(), pages.CreatePageInput{
			UserID: newFixtureUserID("system"), Title: "Docs", Slug: newFixtureSlug("docs"), Kind: sectionKind(),
		})
		Expect(err).To(Succeed())
		syncPage, err := createUC.Execute(context.Background(), pages.CreatePageInput{
			UserID: newFixtureUserID("system"), ParentID: pageIDPtr(docs.Page.ID), Title: "Sync Page", Slug: newFixtureSlug("sync"), Kind: pageKind(),
		})
		Expect(err).To(Succeed())
		syncSection, err := createUC.Execute(context.Background(), pages.CreatePageInput{
			UserID: newFixtureUserID("system"), ParentID: pageIDPtr(docs.Page.ID), Title: "Sync Section", Slug: newFixtureSlug("sync"), Kind: sectionKind(),
		})
		Expect(err).To(Succeed())
		if _, err := createUC.Execute(context.Background(), pages.CreatePageInput{
			UserID: newFixtureUserID("system"), ParentID: pageIDPtr(syncSection.Page.ID), Title: "Child", Slug: newFixtureSlug("child"), Kind: pageKind(),
		}); err != nil {
			Expect(err).To(Succeed())
		}
		pageRef, err := createUC.Execute(context.Background(), pages.CreatePageInput{
			UserID: newFixtureUserID("system"), Title: "Page Ref", Slug: newFixtureSlug("page-ref"), Kind: pageKind(),
		})
		Expect(err).To(Succeed())
		sectionDescendantRef, err := createUC.Execute(context.Background(), pages.CreatePageInput{
			UserID: newFixtureUserID("system"), Title: "Section Descendant Ref", Slug: newFixtureSlug("section-descendant-ref"), Kind: pageKind(),
		})
		Expect(err).To(Succeed())
		pageRefContent := "[Sync page](/docs/sync.md)"
		if _, err := updateUC.Execute(context.Background(), pages.UpdatePageInput{
			UserID: newFixtureUserID("system"), ID: pageID(pageRef.Page.ID), Version: pageVersion(pageRef.Page.Version()), Title: pageRef.Page.Title, Slug: slug(pageRef.Page.Slug), Content: &pageRefContent, Kind: pageKind(),
		}); err != nil {
			Expect(err).To(Succeed())
		}
		sectionDescendantRefContent := "[Sync child](/docs/sync/child.md)"
		if _, err := updateUC.Execute(context.Background(), pages.UpdatePageInput{
			UserID: newFixtureUserID("system"), ID: pageID(sectionDescendantRef.Page.ID), Version: pageVersion(sectionDescendantRef.Page.Version()), Title: sectionDescendantRef.Page.Title, Slug: slug(sectionDescendantRef.Page.Slug), Content: &sectionDescendantRefContent, Kind: pageKind(),
		}); err != nil {
			Expect(err).To(Succeed())
		}
