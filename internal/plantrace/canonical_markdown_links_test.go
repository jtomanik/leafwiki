...
var canonicalMarkdownLinksPlanScenarioCoverage = []canonicalPlanScenarioCoverage{
	...
	{"Relative section links are resolved from the source file directory", evidence("internal/links/link_service_resolution_test.go", "resolves relative section links from the source file directory")},
	...
	{"Relative link cannot escape the workspace root", evidence("internal/workspacesync/service_canonical_rollback_test.go", "leaves invalid canonical links unchanged and reports invalid link issues")},
	...
	{"Refactor preview reports conflicts without mutating content", evidence("internal/wiki/pages/pages_refactor_apply_test.go", "leaves incoming links unchanged when rename target conflicts")},
	...
	{"Old extensionless section link remains extensionless", evidence("internal/workspacesync/service_changed_snapshot_test.go", "canonicalizes section trailing slashes without repeat revisions")},
	{"Unresolved old extensionless page link becomes validation error", evidence("internal/workspacesync/service_canonical_rollback_test.go", "keeps unresolved legacy links in content and reports a broken link issue")},
	...
	{"Relative old page link migrates to relative .md", evidence("internal/workspacesync/service_canonical_links_test.go", "migrates relative legacy page links while preserving canonical relative links")},
	{"Existing canonical .md page link is not rewritten", evidence("internal/workspacesync/service_canonical_links_test.go", "migrates relative legacy page links while preserving canonical relative links")},
	{"Ambiguous extensionless link is left as validation error", evidence("internal/workspacesync/service_ambiguous_links_test.go", "keeps ambiguous extensionless links unchanged and reports an ambiguity issue")},
	...
	{"Explicit README.md page link stays a page when index.md exists", evidence("internal/http/router_split10_test.go", "serves README.md as a section fallback only when no explicit page exists")},
	{"Migration is idempotent", evidence("internal/workspacesync/service_canonical_rollback_test.go", "does not create another migration revision after links are canonical")},
	{"Migration writeback is captured in revision history", evidence("internal/workspacesync/service_canonical_links_test.go", "keeps raw incoming content and canonical writeback revisions")},
	{"Migration write failure reports sync validation state without losing raw content", evidence("internal/workspacesync/service_canonical_rollback_test.go", "reports rewrite failure before derived indexes rebuild")},
	{"Canonical .md page link indexes as outgoing link", evidence("internal/links/link_service_resolution_test.go", "resolves canonical relative page markdown links from the source file directory")},
	{"Canonical section link indexes as outgoing link", evidence("internal/links/link_service_resolution_test.go", "prefers canonical section targets over same-basename page twins for extensionless links")},
	...
	{"Broken canonical .md page link is reported as broken", evidence("internal/links/link_service_resolution_test.go", "returns broken targets with normalized paths for missing destinations")},
	...
	{"Duplicate syntaxes do not create duplicate target identities after migration", evidence("internal/workspacesync/service_canonical_links_test.go", "indexes duplicate legacy and canonical syntaxes as a single healthy target")},
	...
	{"MCP agent context returns canonical examples", evidence("internal/wiki/mcp/mcp_integration_registration_context_test.go", "returns agent-ready context state and canonical link examples")},
